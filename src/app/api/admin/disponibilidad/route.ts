...
import { AdminAvailabilityError, getAdminAvailability } from "@/server/services/admin-availability";

export const dynamic = "force-dynamic";
const HEADERS = { "Cache-Control": "private, no-store" };

export async function GET(request: Request): Promise<Response> {
  return handleAdminAvailabilityRequest(request);
}

export async function handleAdminAvailabilityRequest(
  request: Request,
  authenticate: typeof isAdminAuthenticated = isAdminAuthenticated,
  load: typeof getAdminAvailability = getAdminAvailability,
): Promise<Response> {
  ...
  if (!date) return NextResponse.json({ error: { code: "INVALID_DATE", message: "La fecha no es válida." } }, { status: 400, headers: HEADERS });
  try {
    return NextResponse.json(await load(date), { status: 200, headers: HEADERS });
  } catch (error) {
